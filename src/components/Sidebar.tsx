...
const Sidebar = () => {
  ...
  return <UISidebar className={`border-r min-h-screen transition-all duration-300 bg-white ${expanded ? "w-64" : "w-20"} flex flex-col fixed z-30 h-screen overflow-hidden`}>
      <Header className="\n\"flex items-center justifica-between p-4 border-b h-16 py-[41px] bg-white\"\n">
        <div className="flex items-center">
          {expanded && <Logo />}
        </div>
        <Trigger className="p-2 rounded-md hover:bg-[#e6f0ff] hover:text-[#0066FF] text-gray-700" icon={expanded ? <ChevronLeft size={20} /> : <ChevronRight size={20} />} />
      </Header>
